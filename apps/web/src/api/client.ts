 ...
 * @returns the data at the endpoint
 */
const postAt = async <T>(endpoint: string, options?: GetOptions) => {
  const session = await fetchAuthSession({ forceRefresh: true })
 ...
 * @returns the data at the endpoint
 */
const deleteAt = async <T>(endpoint: string, options?: GetOptions) => {
  const session = await fetchAuthSession({ forceRefresh: true })
...
export const postAtApiV2 = <T>(endpoint: string, options?: GetOptions) =>
  postAt<T>(`/api/v2${endpoint}`, options)

export const deleteAtApiV2 = <T>(endpoint: string, options?: GetOptions) =>
  deleteAt<T>(`/api/v2${endpoint}`, options)
