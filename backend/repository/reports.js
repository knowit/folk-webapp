...
reports = [
    ...
    },
    {
      ...
      "queryString": "select c.text as category, avg(a.motivation) as avg_motivation from kompetansekartlegging_categories as c inner join kompetansekartlegging_questions as q on q.categoryid = c.id inner join kompetansekartlegging_answers as a on a.questionid = q.id group by c.text",
      "tables": [
    ...
    },
    {
      ...
      "queryString": "SELECT emp.user_id, navn, customer, description, year_from, year_to, month_from,  month_to FROM cv_partner_project_experience AS exp JOIN (SELECT user_id, navn FROM cv_partner_employees) emp ON exp.user_id = emp.user_id",
      "tables": [
        "cv_partner_employees",
        "cv_partner_project_experience"
      ],
