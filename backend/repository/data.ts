...
export const projectsCustomerReports = ({
  parameters: { customer } = {},
}: ReportParams) => [
  {
...
export const projectsCustomer = async (response: ProjectCustomerResponse) => {
  const allTotalHours = response.data.reduce(
    (m, { work_order, total_hours }) =>
      m.set(work_order, (m.get(work_order) || 0) + total_hours),
    new Map()
  )
...
export const customerHoursBilledReports = ({
  parameters: { customer } = {},
}: ReportParams) => [
  {
...
export const customerMotivationKnowledgeReports = ({
  parameters: { customer } = {},
}: ReportParams) => [
  {
...
export const customerSiteTableReports = ({
  parameters: { customer } = {},
}: ReportParams) => [
  {
    reportName: 'employeeInformation',
