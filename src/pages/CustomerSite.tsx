  ...
  return (
    <div className={classes.container}>
      <div className={classes.header}>
        <ArrowBack className={classes.arrowBack} onClick={() => routeOnClick()} />
        <h1>{customerName}</h1>
      </div>
      ...
      <div className={classes.factBox}>
        ...
        {projects &&
        <div>
          <span className={classes.bold}>Antall timer brukt hos kunden: </span>
          {projects.map((row: { rowData: any[] }) =>
            row.rowData[3]).reduce((a: number, b: number) => a + b, 0)} {/* rowData[3] is total hours per project */}
        </div>}
      </div>
      <Grid
        ...
        <GridItem>
          <DDItem
            url={'/api/data/customerMotivationKnowledge?customer=' + customerName}
