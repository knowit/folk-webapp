...
const getDisplayValue = (value: ValueType) : string => typeof(value) === 'string' ? value : value.displayValue
const getValue = (value: ValueType) : any => typeof(value) === 'string' ? value : value.value
...
export default function DropdownPicker({
    ...
    const width = measureTextWidth(
        values.map(x => getDisplayValue(x))
        .reduce((prev, next) => prev.length > next.length ? prev : next), '16pt arial') + 46
